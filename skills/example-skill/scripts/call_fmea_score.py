...
Template script for `example-skill` (#270). It is the orchestration half of the invariant:
it starts the MCP server as a subprocess, calls one tool, and prints what came back. It
imports `fastmcp` plus stdlib only — no engine package, no arithmetic. If you find yourself
needing a number this script does not already receive, the fix is a tool call, not a formula.

    python skills/example-skill/scripts/call_fmea_score.py 9 8 5
"""
