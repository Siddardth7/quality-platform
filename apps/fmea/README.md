...
The AIAG/VDA FMEA Handbook (2019) replaced RPN-based prioritization with **Action Priority (AP)**: a published lookup table that maps every S×O×D combination directly to **High / Medium / Low**, weighting **Severity first, then Occurrence, then Detection**. Crucially, AP does *not* simply track severity: at the same Occurrence and Detection, higher Severity never lowers the AP (e.g. `S=10,O=4,D=7` → High vs `S=5,O=4,D=7` → Medium), yet a rare, well-detected high-severity failure is still **Low** (`S=10,O=2,D=2` → Low) — there's little occurrence or detection left to act on. That nuance is exactly what a multiply-the-three-numbers RPN misses.

This tool implements the **complete published table** (no approximation) in `fmea_app/ap_engine.py`. A sidebar toggle switches the prioritization basis **RPN ↔ AP**; both columns stay visible, and the choice flows into ranking, tiering, the critical-items view, and the Excel/PDF/CSV exports. The table is transcribed from the AIAG & VDA FMEA Handbook (2019) and verified cell-by-cell against it for all 1,000 S/O/D combinations, cross-checked against an external peer-reviewed case study — see [`docs/FMEA_methodology_notes.md`](docs/FMEA_methodology_notes.md) §4.2 and [`docs/ASSUMPTIONS_LOG.md`](docs/ASSUMPTIONS_LOG.md) Rule 7.
