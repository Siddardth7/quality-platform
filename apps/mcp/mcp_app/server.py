"""The quality-platform MCP server: FastMCP app, stdio, meta + FMEA + SPC + MSA tools.
...
(Cp/Cpk/Pp/Ppk + CIs) and the stability gate — and M1-5 (#264) the MSA Gage R&R tool. Every
later domain tool (Control Plan, SECOM) lands on this same ``app`` object in this same module
— the CI coverage gate targets ``mcp_app.server`` only, so a separate tools module would
