...
The scale bundled as the default here is **AIAG FMEA-4 (4th Ed., 2008)** — a different edition from the one the shipped AP table (RULE 7) was transcribed from. So an AP result produced from FMEA-4-anchored integers is only as standards-correct as that pairing allows. Shipping the 2019 handbook's own S/O/D tables as the default scale (with FMEA-4 retained as a selectable legacy option) is **tracked in #256**; it is not attempted here because the handbook's PFMEA Occurrence/Detection tables are OCR-mangled in the available primary-source conversion and cannot be transcribed verbatim without fabrication risk.
